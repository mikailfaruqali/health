            ...
            <div class="card rounded-2xl p-4 sm:p-6 lg:p-8 mb-6 fade-in">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div>
                        <div class="flex items-center">
                            <div class="w-10 h-10 sm:w-12 sm:h-12 lg:w-14 lg:h-14 rounded-xl bg-indigo-100 flex items-center justify-center mr-3 sm:mr-4">
                ...
                <div class="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-100">
                    <div class="flex flex-col sm:flex-row gap-3">
                        <a href="?run=all" class="btn-primary px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg font-medium text-center text-sm sm:text-base">
                            <i class="fas fa-play-circle mr-2"></i>
                            Run All Checks
                        </a>
                        @if(!empty($results))
                            <a href="{{ url()->current() }}" class="btn-secondary px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg font-medium text-center text-sm sm:text-base">
            ...
            @if(empty($results))
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
                    @foreach($checks as $index => $check)
                        <a href="?run={{ $check['class'] }}" class="check-card card rounded-xl p-4 sm:p-6 fade-in" style="animation-delay: {{ $index * 0.05 }}s">
                            <div class="flex items-start justify-between mb-3 sm:mb-4">
                                <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center">
                                    <i class="fas fa-{{ $index % 5 == 0 ? 'database' : ($index % 5 == 1 ? 'shield-alt' : ($index % 5 == 2 ? 'chart-line' : ($index % 5 == 3 ? 'server' : 'cog'))) }} text-gray-600"></i>
                                </div>
                ...
                <div class="space-y-4">
                    @foreach($results as $result)
                        <div class="card rounded-xl overflow-hidden fade-in">
                            ...
                            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 {{ $result['status'] === 'success' ? 'bg-green-50' : ($result['status'] === 'warning' ? 'bg-amber-50' : 'bg-red-50') }}">
                                <div class="flex flex-col gap-3">
                                    <!-- Top Row: Name and Status -->
                                    <div class="flex items-start justify-between">
                                        <div class="flex items-center flex-1">
                                            <span class="status-dot {{ $result['status'] }} mr-3 flex-shrink-0"></span>
                                            <h3 class="font-semibold text-gray-900 text-sm sm:text-base break-words">{{ $result['name'] }}</h3>
                                        </div>
                                        <a href="?run={{ array_search($result, $results) }}" class="text-indigo-600 hover:text-indigo-700 text-xs sm:text-sm font-medium flex-shrink-0 ml-2">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="text-xs px-2 py-1 rounded-full {{ $result['status'] === 'success' ? 'badge-success' : ($result['status'] === 'warning' ? 'badge-warning' : 'badge-error') }}">
                                            @if($result['status'] === 'success')
                                                <i class="fas fa-check mr-1"></i>Passed
                                            @elseif($result['status'] === 'warning')
                                                <i class="fas fa-exclamation-triangle mr-1"></i>{{ $result['count'] }} {{ $result['count'] == 1 ? 'Issue' : 'Issues' }}
                                            @else
                            ...
                            <div class="p-4 sm:p-6">
                                @if($result['status'] === 'error')
                                    <div class="bg-red-50 border border-red-200 rounded-lg p-3 sm:p-4">
                                        <div class="flex">
                                            ...
                                            <p class="text-red-700 text-xs sm:text-sm break-words">{{ $result['message'] }}</p>
                                        </div>
                                    </div>
                                ...
                                @else
                                    @php
                                        $headers = array_keys((array)$result['data'][0]);
                                    @endphp
                                            ...
                                            <thead>
                                                <tr class="border-b border-gray-200">
                                                    @foreach($headers as $header)
                                                        <th class="text-left py-3 px-3 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                                ...
                                                @foreach($result['data'] as $row)
                                                    <tr class="hover:bg-gray-50 transition">
                                                        @foreach((array)$row as $key => $value)
                                                            <td class="py-3 px-3 {{ str_contains($key, 'difference') && $value != 0 ? 'text-red-600 font-semibold' : 'text-gray-700' }}">
                                                                {{ $value }}
                                                            </td>
                                                        @endforeach
                                                    </tr>
                                            ...
                                            @foreach($result['data'] as $index => $row)
                                                <div class="mobile-card-item">
                                                    <div class="flex items-center justify-between mb-2">
                                                        <span class="text-xs font-semibold text-gray-700">Item #{{ $index + 1 }}</span>
                                                    ...
                                                    <div class="grid grid-cols-1 gap-2">
                                                        @foreach((array)$row as $key => $value)
                                                            <div class="flex flex-col">
                                                                <div class="mobile-label">{{ str_replace('_', ' ', $key) }}</div>
                                                                <div class="mobile-value {{ str_contains($key, 'difference') && $value != 0 ? 'text-red-600 font-semibold' : '' }}">
                                                                    {{ $value ?: '-' }}
                                                                </div>
                                                            </div>
                                                        @endforeach
    ...
    <footer class="footer">
        <div class="px-4 py-3">
            <div class="max-w-7xl mx-auto">
                <div class="flex flex-col sm:flex-row items-center justify-center sm:justify-between gap-2 text-center sm:text-left">
                    ...
                    <a href="https://snawbar.com" target="_blank" class="text-xs sm:text-sm text-indigo-600 hover:text-indigo-700 font-medium inline-flex items-center">
